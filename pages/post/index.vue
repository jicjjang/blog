...
<script>
  ...
  export default {
    ...
    computed: {
      ...
      postContent() {
        return Marked(require(`~/static/posts/${this.postName ? this.parsedPostName : this.postList[0].path.split('post/')[1]}.md`))
      }
    },
    data () {
      return {
    ...
    created() {
      this.parsedPostName = this.postName ? this.postName : this.postList[0].path.split('post/')[1];

      for (let i in this.postList) {
        if (this.postList[i].path.split('post/')[1] === this.parsedPostName) {
